...
package client
var allLatency time.Duration = 0
var allCommands int = 0
...
loop:
	for {
		...
		var issuerID_tmp string = "XXXXXX"
		var subjectID_tmp string = "XXXXXX"
		issuerRC_tmp := []byte{"XXXXXX"}
		cert_tmp := [][]byte{}
		for i := 0; i<= 1; i++ {
			row1 := []byte{"XXXXXX"}
			cert_tmp = append(cert_tmp, row1)
		}

		var CIPhash_tmp := []byte{"XXXXXX"}
		var CIPsig_tmp := []byte{"XXXXXX"}
		cmd := &clientpb.Command{
			ClientID:       uint32(c.opts.ID()),
			SequenceNumber: num,
			Data:           data[:n],
			IssuerID:       issuerID_tmp,
			subjectID:      subjectID_tmp,
			IssuerRC:       issuerRC_tmp,
			Cert:           cert_tmp,
			NotBefore:      timestamppb.New(time.Now()),
			NotAfter:       timestamppb.New(time.Now()),
			CIPhash:        CIPhash_tmp,
			CIPsig:         CIPsig_tmp,
		}
...
func (c *Client) handleCommands(ctx context.Context) (executed, failed, timeout int) {
	for {
		...
		allLatency = allLatency + duration

		duration_list[allCommands] = duration
		allCommands = allCommands + 1
